  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-slate-800 px-6 py-4 flex items-center gap-3">
        <Link href={`/topic/${topic.id}/books`} className="text-slate-400 hover:text-white transition-colors text-sm">
          ← Books
        </Link>
        <span className="text-slate-700">|</span>
        <div>
          <h1 className="text-base font-semibold leading-tight">{book.title}</h1>
          <p className="text-xs text-slate-500">{book.author} · {book.edition}</p>
        </div>
      </header>
        ...
        <div className="space-y-3">
          {book.parts.map((part) => (
            part.status === "available" ? (
              <Link key={part.id} href={`/topic/${topic.id}/books/${book.id}/${part.id}`}>
                <div className="group bg-slate-900 border border-slate-800 hover:border-slate-600 hover:bg-slate-800/60 transition-all rounded-xl p-5 cursor-pointer">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className={`text-sm font-semibold text-white ${topic.accentHover} transition-colors`}>{part.title}</p>
                      <p className="text-xs text-slate-500 mt-1 leading-relaxed">{part.subtitle}</p>
                      <div className="flex gap-3 mt-3">
                        <span className={`text-xs ${topic.badge} px-2 py-0.5 rounded-full`}>{part.questions} recall questions</span>
                        <span className="text-xs text-blue-400 bg-blue-950/50 px-2 py-0.5 rounded-full">{part.mcq} MCQ pool</span>
                      </div>
                    </div>
                    <span className="text-slate-600 group-hover:text-white transition-colors text-lg mt-1">→</span>
                  </div>
                </div>
              </Link>
            ) : (
